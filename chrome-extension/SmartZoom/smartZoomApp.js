  ...
  $('*').bind('mousedown', function (e) {
    ...
    if (e.button == 1) {
      ...
      if (currentZoomElement != null) {
        currentZoomElement.zoomTo({ targetsize: 1, scalemode: 'width', duration: 300, closeclick: true });
        currentZoomElement = null;
      } else {
        var element = $(this);
        console.log(element);
        if(element!=null && element.text().length>0 &&element.context != body.context){
          element.zoomTo({ targetsize: 0.75, scalemode: 'width', duration: 300, closeclick: true, root: $(document.body) });
